...
def validate(model):
    ...
    with torch.no_grad():
        for i, (inputs, labels, sample_fnames) in enumerate(data_loader):
            ...
            # print_error_img_name(inputs, labels, preds, sample_fnames, class_name, writer)
...
def print_error_img_name(inputs, labels, preds, sample_fnames, class_name, writer):
    for j in range(inputs.size()[0]):
        original_file_name = sample_fnames[j].rsplit('/', 1)[1].replace('.', '_')
        res_list = re.split('_', original_file_name)

        row = [res_list[1], res_list[2], labels[j].item(), preds[j].item()]
        writer.writerow(row)
        if preds[j] != labels[j]:
            original_file_name = sample_fnames[j].rsplit('/', 1)[1].replace('.', '_')
            res_list = re.split('_', original_file_name)
